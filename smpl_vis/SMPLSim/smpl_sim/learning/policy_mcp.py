...
class PolicyMCP(Policy):
    ...
    def live_plotter(self, w, identifier='', pause_time=0.00000001):
        ...
        for i in range(7):
            # after the figure, axis, and line are created, we only need to update the y-model
            self.y_vecs[i][-1] = w[0][i]
            self.lines[i].set_ydata(self.y_vecs[i])
            # this pauses the model so the figure/axis can catch up - the amount of pause can be altered above
